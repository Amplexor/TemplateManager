...
public class TemplateDesignFindPageServlet extends SlingAllMethodsServlet {
	private final static Logger logger = LoggerFactory.getLogger(TemplateDesignFindPageServlet.class);
    ...
    protected void doGet(SlingHttpServletRequest request, SlingHttpServletResponse response) throws ServletException, IOException {
        ...
        List<Hit> pages = findPageWithTemplate(path);
        ...
        if (pages == null || pages.isEmpty()) {
            ...
            try {
                ...
                Hit pageHit = pages.get(0);
                pageMap.put("path", pageHit.getPath());
                
                results.add(pageMap);
                gson.toJson(results,jsonType,jsonWriter);
                
            } catch (RepositoryException ex) {
                logger.error("Could not get page with the template", ex);
            } finally {
                if (jsonWriter != null) {
                    jsonWriter.close();
                }
    ...
    private List<Hit> findPageWithTemplate(String templatePath) {
        try {
            ...
            map.put("property.value", templatePath);
            map.put("p.offset", "0"); 
            map.put("p.limit", "1");
            ...
            try {
                ...
                if(parsysNode != null) {
                    ...
                    if(parsysNode.hasProperty("components")) {
                        Value[] components = parsysNode.getProperty("components").getValues();
